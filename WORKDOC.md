...
앞으로 신규 기능을 추가하더라도 현재 MYPPL의 큰 구조는 임의로 변경하지 않는다.

보존 대상:

- 현재 게시판 중심 구조: `판매자공유핫이슈`, `구매자공유핫이슈`, `커뮤니티`
- 현재 Admin 중심 운영 구조
- PostgreSQL을 운영 데이터 원본으로 두는 정책
- VM + Docker Compose 기준 검수/운영 방식
- 로그인/JWT/권한 구조
- 게시글, 댓글, 이미지, 상품 순위 노출의 기존 저장/표시 흐름
- Bootstrap은 최초 설치용으로만 사용하는 정책

신규 기능 추가 방식:

- 기존 구조를 대체하지 않고 옆으로 확장한다.
- 메뉴/페이지/API/model을 추가하되 기존 게시판/메인/로그인 기능을 건드리지 않는다.
- 기존 데이터 마이그레이션이 필요한 경우 반드시 별도 계획과 승인 후 진행한다.
- 기능 추가 후 `scripts/auth_stability_harness.sh vm`으로 로그인 안정성을 확인한다.
- 신규 기능 검증 실패가 기존 기능 장애로 이어지지 않도록 라우트와 설정을 분리한다.
...
### 9.0 방송/라이브 쇼핑 1차 추가 계획

목표:

- 메인 메뉴 가장 우측에 `방송` 메뉴를 추가한다.
- 1차 범위는 자체 방송 인프라가 아니라 `외부 라이브 URL + 자체 채팅 + 상품연동` 방식으로 구현한다.
- 기존 게시판/상품/메인 구조는 유지하고, 방송 기능은 독립 메뉴와 독립 Admin 설정으로 확장한다.
...
- 상단 메뉴 가장 우측에 `방송` 추가
- Admin 설정에서 방송 메뉴 외부 노출 ON/OFF
- Admin 좌측 메뉴에 `방송관리` 또는 `방송관련 셋업` 추가
- 방송 목록 페이지 추가
- 방송 상세 페이지 추가
- 외부 라이브 URL 입력 및 표시
- 자체 실시간 채팅 기능 추가
- 방송별 연결 상품 목록 추가
- 방송 상태값 추가: `예정`, `라이브`, `종료`, `숨김`
- 방송 종료 후 상세 페이지를 다시보기/기록 페이지로 유지할 수 있는 구조 준비

1차에서 제외:

- 자체 RTMP ingest 서버
- 자체 HLS transcoding 서버
- 방송 녹화/다시보기 자동 저장
- 결제/정산
- 쿠폰/타임딜 자동화
- 대규모 CDN 최적화
...
- 영상: YouTube Live, Vimeo, Mux, AWS IVS 등 외부 라이브 URL 또는 embed URL을 우선 사용한다.
- 채팅: Django Channels + Redis 기반 WebSocket을 사용한다.
- 상품연동: 기존 게시글/상품 카드 데이터를 참조한다. 새로운 상품 저장 구조를 먼저 만들지 않는다.
- 권한: 로그인 사용자만 채팅 가능하게 하고, 비로그인 사용자는 시청만 허용할지 별도 설정으로 둔다.
- 운영자 권한: 관리자/운영자는 채팅 삭제, 사용자 차단, 방송 숨김 처리가 가능해야 한다.
...
- 방송 메뉴 OFF 시 상단 메뉴에서 `방송`이 사라져야 한다.
- 방송 메뉴 ON 시 메인 메뉴 가장 우측에 `방송`이 표시되어야 한다.
- 외부 라이브 URL을 저장하면 방송 상세에서 영상이 표시되어야 한다.
- 로그인 사용자는 채팅을 보낼 수 있어야 한다.
- 새 채팅은 새로고침 없이 같은 방송방 사용자에게 표시되어야 한다.
- 운영자는 채팅을 삭제/숨김 처리할 수 있어야 한다.
- 방송에 연결한 상품은 방송 상세에 노출되어야 한다.
- 방송 기능 추가 후 기존 로그인, 메인, 게시판 작성, 이미지 업로드는 깨지면 안 된다.
...
- `admin / admin` 로그인 후 방송관리 진입
- 방송 메뉴 ON/OFF 변경
- 방송 생성 후 `/live` 목록 노출 확인
- 방송 상세에서 영상/채팅/상품 표시 확인
- `buy / buy`, `sell / sell`로 채팅 권한 확인
...
7. 방송 1차 기능 설계 확정: 외부 라이브 URL, 자체 채팅, 상품연동, Admin ON/OFF
8. 방송 메뉴 추가 전 기존 로그인/게시판/메인 기능 기준 스냅샷 확인
